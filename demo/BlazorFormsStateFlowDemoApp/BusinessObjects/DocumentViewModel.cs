...
namespace BlazorFormsStateFlowDemoApp.BusinessObjects
{
    public class DocumentViewModel
    {
        ...
        {
            if (AssignedTransactions.Any())
            {
                var rnd = new Random();
                var index = rnd.Next(AssignedTransactions.Count);
                var tr = AssignedTransactions[index];

                // set dummy change value to cache to pass trigger 
                var data = DocumentApi._dataCache.List;
                data.Clear();
                data.Add(new ModelPropertyDetails { Id = tr.Document.TransactionId, Bindnig = "AssignedUser", Value = "background@changed" });

                await TransitionClicked("", null, tr);

                // clear cache to prevent infinite trigger execution 
                data.Clear();
            }
        }
