...
namespace BlazorForms.Platform.Crm.Business.Artel
{
    ...
    public class FormArtelProjectSettings : FlowTaskDefinitionBase<ArtelProjectSettingsModel>
    {
        ...
        [FlowRule(typeof(ProjectRoleNameEmptyRule), FormRuleTriggers.Changed)]
        [FlowRule(typeof(ProjectRoleNameEmptyRule), FormRuleTriggers.ItemChanged)]
    ...
    public class ProjectRoleNameEmptyRule : FlowRuleBase<ArtelProjectSettingsModel>
    {
        public override string RuleCode => "APS-EMPTY-1";

        public override void Execute(ArtelProjectSettingsModel model)
        {
            if (string.IsNullOrWhiteSpace(model.Roles[RunParams.RowIndex].Name))
            {
                Result.ValidationResult = RuleValidationResult.Error;
                Result.ValidationMessage = "Please enter Role Name";
            }
        }
    }
}
