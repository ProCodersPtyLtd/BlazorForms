...
namespace CrmLightDemoApp.Store.Onion.Services.Flow
{
    public class PersonListFlow : ListFlowBase<PersonListModel, FormPersonList>
    {
        ...
        public PersonListFlow(IHighStore store)
        {
            _store = store;
        }
        ...
        {
            var q = _store.GetQuery<PersonModel>();
            var list = await q.ToListAsync(queryOptions);
    ...
    public class FormPersonList : FormListBase<PersonListModel>
    {
        protected override void Define(FormListBuilder<PersonListModel> builder)
        {
            builder.List(p => p.Data, e =>
            {
                ...
                e.Property(p => p.FirstName).Label("First RoleName").Filter(FieldFilterType.TextStarts);
                e.Property(p => p.LastName).Label("Last RoleName").Filter(FieldFilterType.TextStarts);
